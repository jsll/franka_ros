 ...
 */
struct CustomFrankaDataContainerKth {
...
class KthCartesianPoseEffortInterfaceController: public controller_interface::MultiInterfaceController<
 ...
 private:
  std::map<std::string, CustomFrankaDataContainerKth>  arms_data_;  ///< Holds all relevant data for both arms.
   ...
   */
  Eigen::Matrix<double, 7, 1> saturateTorqueRate( const CustomFrankaDataContainerKth& arm_data, const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
   ...
   */
  void updateArm(CustomFrankaDataContainerKth& arm_data, const string& arm_id, const ros::Duration& period);
   ...
   */
  void startingArm(CustomFrankaDataContainerKth& arm_data);
