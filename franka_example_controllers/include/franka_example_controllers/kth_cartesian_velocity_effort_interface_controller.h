 ...
 */
struct CustomFrankaDataContainerKthVel {
  ...
  Planner_vel planner_vel_; // velocity planner in cartesian space
...
class KthCartesianVelocityEffortInterfaceController: public controller_interface::MultiInterfaceController<
 ...
 private:
  std::map<std::string, CustomFrankaDataContainerKthVel>  arms_data_;  ///< Holds all relevant data for both arms.
   ...
   */
  Eigen::Matrix<double, 7, 1> saturateTorqueRate( const CustomFrankaDataContainerKthVel& arm_data, const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
   ...
   */
  void updateArm(CustomFrankaDataContainerKthVel& arm_data, const string& arm_id, const ros::Duration& period);
   ...
   */
  void startingArm(CustomFrankaDataContainerKthVel& arm_data);
  ...
  void targetVelocityCallback(const geometry_msgs::Twist::ConstPtr& msg, const string& robot_id); 
