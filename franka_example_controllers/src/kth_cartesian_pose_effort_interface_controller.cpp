// KthCartesianPoseEffortInterfaceController for usage in KTH RPL lab

#include <franka_example_controllers/kth_cartesian_pose_effort_interface_controller.h>
#include <cmath>
#include <functional>
#include <memory>
...
inline bool KthCartesianPoseEffortInterfaceController::checkTorqueLimits(const Eigen::Matrix<double, 7, 1>& tau, const string& arm_id){
  for(int i = 0; i < 7; i++){
...
inline  bool KthCartesianPoseEffortInterfaceController::checkDerivativeTorqueLimits(const Eigen::Matrix<double, 7, 1>& dtau, const string& arm_id){
  for(int i = 0; i < 7; i++){
...
inline bool KthCartesianPoseEffortInterfaceController::checkJointLimits(const Eigen::Matrix<double, 7, 1>& q, const string& arm_id){
...
inline bool KthCartesianPoseEffortInterfaceController::checkVelocityJointLimits(const Eigen::Matrix<double, 7, 1>& dq, const string& arm_id){
  for(int i = 0; i < 7; i++){
...
bool KthCartesianPoseEffortInterfaceController::initArm(
  ...
  CustomFrankaDataContainerKth arm_data;
  auto* model_interface = robot_hw->get<franka_hw::FrankaModelInterface>();
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "KthCartesianPoseEffortInterfaceController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "KthCartesianPoseEffortInterfaceController: Exception getting model handle from "
        "interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "KthCartesianPoseEffortInterfaceController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "KthCartesianPoseEffortInterfaceController: Exception getting state handle from "
        "interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "KthCartesianPoseEffortInterfaceController: Error getting effort joint interface from "
        "hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "KthCartesianPoseEffortInterfaceController: Exception getting joint handles: "
          << ex.what());
      return false;
    }
  }
  ...
  if (!node_handle.getParam("Mdpos", Mdpos)) {
    ROS_ERROR_STREAM("KthCartesianPoseEffortInterfaceController: Could not read parameter Mdpos");
    return false;
  }
  if (!node_handle.getParam("Mdor", Mdor)) {
    ROS_ERROR_STREAM("KthCartesianPoseEffortInterfaceController: Could not read parameter Mdor");
    return false;
  }
  ...
  if( !node_handle.getParam("Kp", K_list) || K_list.size() != p_arm )
      ROS_ERROR("KthCartesianPoseEffortInterfaceController: Could not read parameter Kp.");
  else{
  ...
  if (!node_handle.getParam("impedance", impedence_active)) {
    ROS_ERROR_STREAM(
        "KthCartesianPoseEffortInterfaceController: Could not read parameter impedance");
    return false;
  }
  ...
  if (!node_handle.getParam("static", static_traj) ) {
    ROS_ERROR_STREAM(
        "KthCartesianPoseEffortInterfaceController: Could not read parameter static_traj  ");
    return false;
  }
...
bool KthCartesianPoseEffortInterfaceController::waitForInitialTransform(const tf::TransformListener& listener, tf::StampedTransform& transform, const string& frame1, const string& frame2){
  try {
    if (listener.waitForTransform(frame1, frame2, ros::Time(0), ros::Duration(4.0))) {
      ...
      ROS_ERROR( "KthCartesianPoseEffortInterfaceController: Failed to read transform from %s to %s. Aborting init!", frame1.c_str(), frame2.c_str());
      return false;
    }
  } catch (tf::TransformException& ex) {
    ROS_ERROR("KthCartesianPoseEffortInterfaceController: %s", ex.what());
    return false;
  }
  return true; 
}
...
bool KthCartesianPoseEffortInterfaceController::init(hardware_interface::RobotHW* robot_hw,
  ...
  if (!node_handle.getParam("arm_id", arm_id_)) {
    ROS_ERROR_STREAM(
        "KthCartesianPoseEffortInterfaceController: Could not read parameter left_arm_id_");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "KthCartesianPoseEffortInterfaceController: Invalid or no left_joint_names parameters provided, aborting controller init!");
    return false;
  }
  ...
  boost::function<void(const geometry_msgs::PoseStamped::ConstPtr&)> callbackTarget =
      boost::bind(&KthCartesianPoseEffortInterfaceController::targetPoseCallback, this, _1, arm_id_);
...
void KthCartesianPoseEffortInterfaceController::startingArm(CustomFrankaDataContainerKth& arm_data){
...
void KthCartesianPoseEffortInterfaceController::starting(const ros::Time& /*time*/) {
...
void KthCartesianPoseEffortInterfaceController::update(const ros::Time& /*time*/,
...
void KthCartesianPoseEffortInterfaceController::updateArm(CustomFrankaDataContainerKth& arm_data, const string& arm_id, const ros::Duration& period){
  // get state variables
...
Eigen::Matrix<double, 7, 1> KthCartesianPoseEffortInterfaceController::saturateTorqueRate(
    const CustomFrankaDataContainerKth& arm_data,
...
void KthCartesianPoseEffortInterfaceController::getDesiredTrajectory(bool stationary, double time, const Eigen::Matrix<double, 6, 1>& init_x, Eigen::Matrix<double, 6, 1>& xd, Eigen::Matrix<double, 6, 1>& dxd, Eigen::Matrix<double, 6, 1>& ddxd){
  if (stationary){
...
void KthCartesianPoseEffortInterfaceController::getStationaryTrajectory(double time, const Eigen::Matrix<double, 6, 1>& init_x, Eigen::Matrix<double, 6, 1>& xd, Eigen::Matrix<double, 6, 1>& dxd, Eigen::Matrix<double, 6, 1>& ddxd){
  xd = init_x; 
  dxd.setZero(); 
  ddxd.setZero(); 
}

void KthCartesianPoseEffortInterfaceController::getSinTrajectory(double time, const Eigen::Matrix<double, 6, 1>& init_x, Eigen::Matrix<double, 6, 1>& xd, Eigen::Matrix<double, 6, 1>& dxd, Eigen::Matrix<double, 6, 1>& ddxd){
  double period = 7.0; 
...
void KthCartesianPoseEffortInterfaceController::targetPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg, const string& robot_id){
  ...
  cout << "CALLBACK, des quat (panda_table_frame frame) " << o_w_d.x() << " " << o_w_d.y() << " " << o_w_d.z() << " " <<o_w_d.w()<<endl; 
  cout << "CALLBACK, des R (panda_table_frame frame) " << endl; 
  cout << R << endl; 
...
void KthCartesianPoseEffortInterfaceController::analyticalJacobian(Eigen::Matrix<double, 6, 7>& Ja, const Eigen::Matrix<double, 6, 7>& J, const Eigen::Vector3d& eul_angles){
    //cout << "J: "<< J <<endl; 
    Eigen::Matrix<double, 6, 6> Ta; 
    //Ta.setIdentity(); 
    transfAnalyticalJacobian(Ta, eul_angles); 
 ...
 void KthCartesianPoseEffortInterfaceController::transfAnalyticalJacobian(Eigen::Matrix<double, 6, 6>& Ta, const Eigen::Vector3d& ZYX){
  ...
  void KthCartesianPoseEffortInterfaceController::transformCartesianPose(const Eigen::VectorXd& xd_w, Eigen::VectorXd& xd_0, const Eigen::Affine3d& A_0_w, const Eigen::VectorXd& current_eul_0){
  ...
  void KthCartesianPoseEffortInterfaceController::getContinuousEulerAngles(Eigen::Matrix<double, 3, 1>& eul, const Eigen::Matrix<double, 3, 1>& previous_eul ){
...
double KthCartesianPoseEffortInterfaceController::sign(double n){
	if (n > 0){
		return 1; 
	}else if (n < 0){
		return -1; 
...
void KthCartesianPoseEffortInterfaceController::R2Eul(const Eigen::MatrixXd& R, Eigen::Vector3d& eul){
...
void KthCartesianPoseEffortInterfaceController::Eul2R(Eigen::MatrixXd& R, const Eigen::Vector3d& eul){
...
void KthCartesianPoseEffortInterfaceController::publishPoseStamped(realtime_tools::RealtimePublisher<geometry_msgs::PoseStamped>& arm_pub, const Eigen::VectorXd& vec_to_pub,const ros::Time& timestamp){
...
void KthCartesianPoseEffortInterfaceController:: publishTime(realtime_tools::RealtimePublisher<std_msgs::Float64>& time_pub, double time){
  if (time_pub.trylock()) {
  ...
  void KthCartesianPoseEffortInterfaceController::publishTau(realtime_tools::RealtimePublisher<geometry_msgs::PoseStamped>& arm_pub, const Eigen::VectorXd& tau, const ros::Time& timestamp){
    if (arm_pub.trylock()) {
...
PLUGINLIB_EXPORT_CLASS(
    franka_example_controllers::KthCartesianPoseEffortInterfaceController,
