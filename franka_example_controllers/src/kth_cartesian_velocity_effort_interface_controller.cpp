// KthCartesianVelocityEffortInterfaceController for usage in KTH RPL lab

#include <franka_example_controllers/kth_cartesian_velocity_effort_interface_controller.h>
#include <cmath>
#include <functional>
#include <memory>
...
inline bool KthCartesianVelocityEffortInterfaceController::checkTorqueLimits(const Eigen::Matrix<double, 7, 1>& tau, const string& arm_id){
  for(int i = 0; i < 7; i++){
...
inline  bool KthCartesianVelocityEffortInterfaceController::checkDerivativeTorqueLimits(const Eigen::Matrix<double, 7, 1>& dtau, const string& arm_id){
  for(int i = 0; i < 7; i++){
...
inline bool KthCartesianVelocityEffortInterfaceController::checkJointLimits(const Eigen::Matrix<double, 7, 1>& q, const string& arm_id){
...
inline bool KthCartesianVelocityEffortInterfaceController::checkVelocityJointLimits(const Eigen::Matrix<double, 7, 1>& dq, const string& arm_id){
  for(int i = 0; i < 7; i++){
...
bool KthCartesianVelocityEffortInterfaceController::initArm(
  ...
  CustomFrankaDataContainerKthVel arm_data;
  auto* model_interface = robot_hw->get<franka_hw::FrankaModelInterface>();
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "KthCartesianVelocityEffortInterfaceController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "KthCartesianVelocityEffortInterfaceController: Exception getting model handle from "
        "interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "KthCartesianVelocityEffortInterfaceController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "KthCartesianVelocityEffortInterfaceController: Exception getting state handle from "
        "interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "KthCartesianVelocityEffortInterfaceController: Error getting effort joint interface from "
        "hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "KthCartesianVelocityEffortInterfaceController: Exception getting joint handles: "
          << ex.what());
      return false;
    }
  }
  ...
  if (!node_handle.getParam("Mdpos", Mdpos)) {
    ROS_ERROR_STREAM("KthCartesianVelocityEffortInterfaceController: Could not read parameter Mdpos");
    return false;
  }
  if (!node_handle.getParam("Mdor", Mdor)) {
    ROS_ERROR_STREAM("KthCartesianVelocityEffortInterfaceController: Could not read parameter Mdor");
    return false;
  }
  ...
  if( !node_handle.getParam("Kp", K_list) || K_list.size() != p_arm )
      ROS_ERROR("KthCartesianVelocityEffortInterfaceController: Could not read parameter Kp.");
  else{
  ...
  if (!node_handle.getParam("impedance", impedence_active)) {
    ROS_ERROR_STREAM(
        "KthCartesianVelocityEffortInterfaceController: Could not read parameter impedance");
    return false;
  }
  ...
  if (!node_handle.getParam("static", static_traj) ) {
    ROS_ERROR_STREAM(
        "KthCartesianVelocityEffortInterfaceController: Could not read parameter static_traj  ");
    return false;
  }
...
bool KthCartesianVelocityEffortInterfaceController::waitForInitialTransform(const tf::TransformListener& listener, tf::StampedTransform& transform, const string& frame1, const string& frame2){
  try {
    if (listener.waitForTransform(frame1, frame2, ros::Time(0), ros::Duration(4.0))) {
      ...
      ROS_ERROR( "KthCartesianVelocityEffortInterfaceController: Failed to read transform from %s to %s. Aborting init!", frame1.c_str(), frame2.c_str());
      return false;
    }
  } catch (tf::TransformException& ex) {
    ROS_ERROR("KthCartesianVelocityEffortInterfaceController: %s", ex.what());
    return false;
  }
  return true; 
}
...
bool KthCartesianVelocityEffortInterfaceController::init(hardware_interface::RobotHW* robot_hw,
  ...
  if (!node_handle.getParam("arm_id", arm_id_)) {
    ROS_ERROR_STREAM(
        "KthCartesianVelocityEffortInterfaceController: Could not read parameter left_arm_id_");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "KthCartesianVelocityEffortInterfaceController: Invalid or no left_joint_names parameters provided, aborting controller init!");
    return false;
  }
  ...
  boost::function<void(const geometry_msgs::Twist::ConstPtr&)> callbackTarget =
      boost::bind(&KthCartesianVelocityEffortInterfaceController::targetVelocityCallback, this, _1, arm_id_);
  ...
  waitForInitialTransform(listener, transform, "panda_table_link", arm_id_ + "_link0"); 
...
void KthCartesianVelocityEffortInterfaceController::startingArm(CustomFrankaDataContainerKthVel& arm_data){
 // compute initial velocity with jacobian and set x_attractor and q_d_nullspace
  ...
  // init planners
  cout <<  " init planner with : qi " <<arm_data.init_x_.transpose() << " dx: " << arm_data.dx_.transpose()*0 <<endl; 
  arm_data.planner_vel_.init(arm_data.init_x_, arm_data.dx_, arm_data.dx_*0, current_time_); //time constant for derivetive?

  //TODO check INIT condition for controler

}

void KthCartesianVelocityEffortInterfaceController::starting(const ros::Time& /*time*/) {
...
void KthCartesianVelocityEffortInterfaceController::updateArm(CustomFrankaDataContainerKthVel& arm_data, const string& arm_id, const ros::Duration& period){
 ...
 // Get Euler angles and check continuity
  ...
  // get curretn step
  arm_data.planner_vel_.getPose(current_time_, xd, dxd, ddxd);
...
Eigen::Matrix<double, 7, 1> KthCartesianVelocityEffortInterfaceController::saturateTorqueRate(
    const CustomFrankaDataContainerKthVel& arm_data,
...
void KthCartesianVelocityEffortInterfaceController::getDesiredTrajectory(bool stationary, double time, const Eigen::Matrix<double, 6, 1>& init_x, Eigen::Matrix<double, 6, 1>& xd, Eigen::Matrix<double, 6, 1>& dxd, Eigen::Matrix<double, 6, 1>& ddxd){
  if (stationary){
...
void KthCartesianVelocityEffortInterfaceController::getStationaryTrajectory(double time, const Eigen::Matrix<double, 6, 1>& init_x, Eigen::Matrix<double, 6, 1>& xd, Eigen::Matrix<double, 6, 1>& dxd, Eigen::Matrix<double, 6, 1>& ddxd){
  xd = init_x; 
  dxd.setZero(); 
  ddxd.setZero(); 
}

void KthCartesianVelocityEffortInterfaceController::getSinTrajectory(double time, const Eigen::Matrix<double, 6, 1>& init_x, Eigen::Matrix<double, 6, 1>& xd, Eigen::Matrix<double, 6, 1>& dxd, Eigen::Matrix<double, 6, 1>& ddxd){
  double period = 7.0; 
...
void KthCartesianVelocityEffortInterfaceController::targetVelocityCallback(const geometry_msgs::Twist::ConstPtr& msg, const string& robot_id){
  ...
  // Define desired velocity on EE
  Eigen::VectorXd des_vel(6);
  des_vel << msg->linear.x, msg->linear.y, msg->linear.z, msg->angular.x, msg->angular.y, msg->angular.z;
  Eigen::VectorXd curr_x(6);
  curr_x.head(3) = curr_pos; 
  curr_x.tail(3) = eul_angles; 
  arm_data.dx_ = arm_data.previous_Ja_ *arm_data.dq_filtered_;

  arm_data.planner_vel_.plan(curr_x, arm_data.dx_, des_vel, current_time_, 0.1); //time constant

  //cout << robot_id << " Desired velocity: "<<des_vel.transpose() << " Current velocity: " << arm_data.dx_.transpose()<<endl; 
 
  
  
}

void KthCartesianVelocityEffortInterfaceController::analyticalJacobian(Eigen::Matrix<double, 6, 7>& Ja, const Eigen::Matrix<double, 6, 7>& J, const Eigen::Vector3d& eul_angles){
    //cout << "J: "<< J <<endl; 
    Eigen::Matrix<double, 6, 6> Ta; 
    //Ta.setIdentity(); 
    transfAnalyticalJacobian(Ta, eul_angles); 
 ...
 void KthCartesianVelocityEffortInterfaceController::transfAnalyticalJacobian(Eigen::Matrix<double, 6, 6>& Ta, const Eigen::Vector3d& ZYX){
  ...
  void KthCartesianVelocityEffortInterfaceController::transformCartesianPose(const Eigen::VectorXd& xd_w, Eigen::VectorXd& xd_0, const Eigen::Affine3d& A_0_w, const Eigen::VectorXd& current_eul_0){
  ...
  void KthCartesianVelocityEffortInterfaceController::getContinuousEulerAngles(Eigen::Matrix<double, 3, 1>& eul, const Eigen::Matrix<double, 3, 1>& previous_eul ){
...
void KthCartesianVelocityEffortInterfaceController::R2Eul(const Eigen::MatrixXd& R, Eigen::Vector3d& eul){
...
void KthCartesianVelocityEffortInterfaceController::Eul2R(Eigen::MatrixXd& R, const Eigen::Vector3d& eul){
...
void KthCartesianVelocityEffortInterfaceController::publishError(realtime_tools::RealtimePublisher<geometry_msgs::PoseStamped>& arm_pub, const Eigen::VectorXd& error,const ros::Time& timestamp){
...
void KthCartesianVelocityEffortInterfaceController::publishPoseStamped(realtime_tools::RealtimePublisher<geometry_msgs::PoseStamped>& arm_pub, const Eigen::VectorXd& vec_to_pub,const ros::Time& timestamp){
...
void KthCartesianVelocityEffortInterfaceController:: publishTime(realtime_tools::RealtimePublisher<std_msgs::Float64>& time_pub, double time){
  if (time_pub.trylock()) {
  ...
  void KthCartesianVelocityEffortInterfaceController::publishTau(realtime_tools::RealtimePublisher<geometry_msgs::PoseStamped>& arm_pub, const Eigen::VectorXd& tau, const ros::Time& timestamp){
    if (arm_pub.trylock()) {
...
PLUGINLIB_EXPORT_CLASS(
    franka_example_controllers::KthCartesianVelocityEffortInterfaceController,
